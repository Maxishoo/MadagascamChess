...
def read_dataset(path_to_games: str, path_to_marks: str, path_to_engine: str, save_path: str, log=False):
    ...
    datasets = []
    with open(path_to_games, 'r') as pgn_file, open(path_to_marks, 'r') as marks:
        i = 1
        while (game := chess.pgn.read_game(pgn_file)) and (mark := read_mark(marks, len(list(game.mainline_moves())))) is not None:
            if log:
                print(f'[GAME-{i}] Reading...')

            features = extract_features(game, engine, (tmp_path_obj / f'{i}.csv').resolve(), log=log)
            datasets.append(pd.concat([features, mark], axis=1))
